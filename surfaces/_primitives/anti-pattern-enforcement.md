...
| **Freeform brainstorm drift** | `_check_no_freeform_brainstorm` — **NARROWED in v3.8.1**: forbidden-conversational-connectives scan only. Length-budget enforcement was removed for the same reason persona-theater was removed in v3.8.0 — per-field character caps trip on legitimate dense content (summary, synthesis_attempt, recommended_resolution_paths/rationale all calibrated up across v3.7.2 / v3.7.6 / would-have-been-v3.8.1 with the remaining 6 budgeted fields statistically next). Connectives ("as we discussed", "circling back") ARE a finite structural detector and retained. | Agents may drift into conversation; the forbidden-connective list catches the explicit drift markers |
...
- **v3.8.0**: `_check_no_persona_theater` REMOVED. Phase 3 exit retro surfaced four false-positive ships (v3.7.0 negation context; v3.7.1 schema agent-reference fields; v3.7.3 parenthetical citation; v3.7.x narrative role description) demonstrating that regex over free text cannot distinguish role-as-actor (legitimate) from role-as-addressee (conversational drift). The structural-detector-exists property fails: every analytical mode produces new shapes of legitimate `@<Role>` mentions. Removal precedent: when the deterministic detector itself produces operator-decision overhead exceeding the value of the behavioral guard, the detector ships the wrong shape.
- **v3.8.1**: Length-budget enforcement REMOVED from `_check_no_freeform_brainstorm` (same lesson, applied one layer down). Phase 3 exit retro surfaced three budget calibrations on consecutive fields (v3.7.2 summary 1500→2500; v3.7.6 synthesis_attempt 1000→1500; recommended_resolution_paths/rationale 800 hit and would have been next). Six other budgeted fields remained statistically inevitable. The detector cannot distinguish "agent producing dense legitimate content" from "agent overflowing into prose" — per-field character caps tripped on real analytical work as readily as on bloat. Forbidden-connective enforcement is RETAINED: small finite list of explicit drift markers ("as we discussed", "circling back") is a clean structural detector. Length budgets remain in agent frontmatter as advisory documentation; the substrate no longer reads them for veto purposes. The original concern (bloat / drift) is prevented by JSON-envelope-only output parsing + the retro-end synthesist's quality observation pass + the forbidden-connective scan.
