...
| [fixer](.claude/agents/fixer.md) | **Read-only-by-contract.** Stall-recovery agent (v3.2-bridge per Aaron 2026-05-26 directive). Diagnoses stalled / blocked / failed tasks and proposes a validator-eligible `recovery_chain` — or escalates via `escalate: true` when judgment is operator-territory. Daemon auto-dispatches when `next_ready_task` returns None and stalls exist with recursion budget remaining (`FIXER_RECURSION_BOUND = 2`). Fourth instance of read-only-by-contract agent pattern. Composes with `recovery-dispatcher` system agent (which validates the proposal via `fnsr_chain_validator` before append-tasks). Opus tier. |
...
| [test-runner](.claude/agents/test-runner.md) | v2.9.0. Runs the configured test suite via subprocess; returns structured pass/fail/skip counts plus first N failures. Subject-project-agnostic — test command via `FNSR_TEST_RUNNER_CMD` env var or `inputs.cmd`. Built-in parsers: `python_unittest`, `npm`, `raw` fallback. |
| [git-committer](.claude/agents/git-committer.md) | v2.9.0. **First substrate agent with externally-visible side effects.** Creates a git commit via subprocess with strict safety defaults — refuses dirty working tree / protected-branch commits / bypass-hooks unless operator explicitly opts in with `bypass_reason` recorded in audit. Two-class failure discrimination: `hook_failure` (operator fixes code) vs `git_command_failure` (operator fixes substrate/environment). See PLAYBOOK §4.10 for the operator-review-before-queuing pattern. |
| [retro-applier](.claude/agents/retro-applier.md) | v3.0-alpha.2. Deterministic merger of analytical-agent proposals into RETRO_STATE.jsonld. Per-mutation CAS semantics; idempotent via @id keys; single audit-chain entry per dispatch. Analog to v2.6.0 applier for code changes, scoped to retro state. |
| [recovery-dispatcher](.claude/agents/recovery-dispatcher.md) | v3.2-bridge per Aaron 2026-05-26 directive. Consumes a `fixer` agent's `recovery_chain` output; validates via `fnsr_chain_validator` (PRED-1 through PRED-6); append-tasks on PASS; escalates on FAIL or on fixer's `escalate: true`. Third system agent with externally-visible side effects (alongside `applier` and `retro-applier`). Daemon's `run_one_cycle` auto-queues a (fixer, recovery-dispatcher) pair when stall + recursion budget detected. |
...
- `_check_no_freeform_brainstorm` — forbidden conversational connectives (length-budget enforcement removed in v3.8.1; budgets remain in agent frontmatter as advisory only)
...
| [state_admin.py](state_admin.py) | Operator CLI for state.jsonld manipulation. v3.3.0 subcommand: `pending [--print]` (emit pending operator decisions as human-readable Markdown to `fnsr.operator_decisions.md`; closes the emission gap so awaiting_operator_decision tasks are operator-discoverable, not just stored). v3.2.2 subcommand: `reset-fixer-attempts <anchor> --reason "..."` (zeros the Fixer recursion bound for an anchor by appending a `fixer_attempts_reset` audit event; daemon's `_count_fixer_attempts` honors it; use after a Fixer-contract patch when prior-cycle attempts should not count). v3.2.1 subcommand: `abandon-stale-fixers [--dry-run]` (bulk-cleanup helper for blocked Fixers emitting deprecated judgment-refusal codes). v2.6.0 subcommands: `reset`, `abandon`, `append-tasks`, `verify`, `status`, `resolve`, `bank`. v2.7.0 subcommands: `transition-banking`, `phase-boundary`, `forward-track create` / `inherit`. v2.8.0 forward-track subcommands: `transition`, `list`, `aging`. v2.9.0 subcommand: `template-sync` (automates the dual-track-workflow `cp -f` sync from source repo to target repo(s)). v3.0-alpha.2 subcommand: `phase-complete-declaration` (operator-authoritative). v3.0 final subcommand groups: `retro init` / `phase-transition` / `vote` / `archive` / `verify` / `list` (retro-surface operations per MAREP v2.2) + `promote-candidate` (deliberate Episodic→Semantic promotion audit event). v3.1.0-bridge subcommand group: `phase implementing` / `demo-released` / `po-satisfied` / `retro-complete` / `drift-reconciled` / `close` / `status` / `history` (Phase Lifecycle Orchestration per [surfaces/_primitives/phase-lifecycle-orchestration.md](surfaces/_primitives/phase-lifecycle-orchestration.md); emits `phase_state_changed` audit events; `close` refuses unless state == drift-reconciled per AP-PLO-2). v3.1.0-bridge subcommand `verify-chain <chain.json>` + `--verify-first` flag on `append-tasks` (Pre-Dispatch Chain Validator per [fnsr_chain_validator.py](fnsr_chain_validator.py); 6 predicates: applier-source-in-deps, Windows-npm-bare, deps-alive, required-inputs, no-id-collisions, no-circular-deps; catches operator-composition defects before they cascade through state.jsonld). Run `python state_admin.py --help`. |
| [fnsr_chain_validator.py](fnsr_chain_validator.py) | v3.1.0-bridge Pre-Dispatch Chain Validator. Pure-Python predicates over a chain JSON + current state.jsonld. Per Aaron 2026-05-25 root-cause analysis: substrate had no third party validating operator chain composition before dispatch, causing repeated cascade rebuilds. Validated against this session's actual cascade JSONs — fires on the real defects that produced v2/v4/v5 cascades. Run `python fnsr_chain_validator.py <chain.json>` or via `state_admin verify-chain`. |
