...
**Closure (2026-05-23):** Event 11 fixed in `fnsr_daemon.py` via fix shape (b) — daemon-side `next_ready_task` predicate. New helper `_architect_ratification_block(task, by_id)` returns `(dep_id, ruling)` when an applier-class task has any upstream architect-ratification dep whose `outputs.ruling != "ratified"`; `next_ready_task` filters those out. Pass 2a now structurally gates Pass 2b at the picker, independent of the architect's emit pattern (the option-(c) architect-contract approach was considered but rejected in favor of (b) because it removes the responsibility from the agent contract entirely — the daemon enforces the spec invariant regardless of how the architect signals). The applier task stays `status=ready` (no destructive mutation); operator-action is required to either abandon, re-dispatch, or escalate to a follow-up Pass 2a. Regression covered by `TestPassRatificationGating` (8 cases: denied/deferred/ratified rulings; non-applier-not-gated; review-mode-not-gated; skip-with-other-candidate; multi-architect-any-denied; no-architect-dep). Full substrate test suite: 483/483 OK. Three observed instances during Round 4 (originally counted "five-plus" — refined to three distinct denials × ~2 conversation-turns-each cleanup) all reproduce as test failures pre-fix.
